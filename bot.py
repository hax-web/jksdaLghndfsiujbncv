...
try:
    import nacl
    print(f"[진단] PyNaCl 로드 성공, 버전: {nacl.__version__}")
except Exception as e:
    print(f"[진단] PyNaCl 로드 실패: {type(e).__name__}: {e}")

try:
    from discord.voice_client import VoiceClient
    print(f"[진단] discord.py VOICE_SUPPORTED: {getattr(__import__('discord.voice_client', fromlist=['VOICE_SUPPORTED']), 'VOICE_SUPPORTED', '확인불가')}")
except Exception as e:
    print(f"[진단] discord.py voice 모듈 확인 실패: {type(e).__name__}: {e}")
...
async def play(interaction: discord.Interaction, 검색어: str):
    ...
    try:
        ...
        await interaction.followup.send(f"음성 채널 접속 실패: `{e}`", ephemeral=True)
        return
    if not vc:
        return
    try:
